...
describe("handleCheckJob", () => {
  ...
  it("does not run test-session checks with overlapping accounts in parallel", async () => {
    vi.spyOn(console, "log").mockImplementation(() => undefined);
    ...
    mocks.checkRunFindUnique.mockImplementation(async ({ where }) => ({
      checkSnapshotKey: where.id,
      checkSnapshotName: where.id,
      durationMs: null,
      id: where.id,
      status: "QUEUED",
      testSessionId: "session_1",
    }));
    mocks.runTestSessionCheck.mockImplementation(async ({ existingRunId }) => {
      activeChecks += 1;
      ...
      return {
        checkKey: existingRunId,
        checkName: existingRunId,
        durationMs: 1,
        runId: existingRunId,
        status: "passed",
      };
    ...
    const firstRun = handleTestSessionCheckJob(createJob("run_1"));
    const secondRun = handleTestSessionCheckJob(createJob("run_2"));

    try {
      await vi.waitFor(() => {
        expect(mocks.runTestSessionCheck).toHaveBeenCalled();
      });
    ...
    expect(mocks.runTestSessionCheck).toHaveBeenCalledTimes(2);
  });
