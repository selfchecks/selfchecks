...
export default async function TraceArtifactPage({ params }: TraceArtifactPageProps) {
  ...
  const token = createTraceAccessToken(runId, artifactId);
  const artifactUrl = `/api/runs/${encodedRunId}/artifacts/${encodedArtifactId}?traceViewer=1&token=${encodeURIComponent(
    token,
  )}`;
